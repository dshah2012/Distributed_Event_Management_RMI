...
public class OttawaClass extends UnicastRemoteObject implements McInterface {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	HashMap<String, HashMap<String, EventDetails>> ottawa_db = new HashMap<String, HashMap<String, EventDetails>>();
	...
	public OttawaClass() throws Exception {
		...
		ottawa_db.put("conferences", new HashMap<String, EventDetails>());
		ottawa_db.put("trade shows", new HashMap<String, EventDetails>());
		ottawa_db.put("seminars", new HashMap<String, EventDetails>());
	...
	public String udp_packet_send(String destServer, String actionType,
		...
		System.out
				.println("Ottawa UDP Server: Sending UDP packet with eventType: "
						+ eventType
						+ " for: "
						+ actionType
						+ " to: "
		...
		try {
			...
			String logMessages = "Ottawa UDP Server: Reply received from the server is: "
					+ retVal;
			System.out.println(logMessages);// print reply
			Helpers.ServerLog("OTW", logMessages);
	...
	public void udp_packet_recv() {
		...
		try {
			...
			byte[] buffer = new byte[1000];// to stored the received data from
											// the client.
			System.out.println("Ottawa UDP Server Started on port: " + portNo);
			...
			while (true) {// non-terminating loop as the server is always in
				...
				String logMessages = "Ottawa UDP Server: Request received from client: "
						+ receivedData;
				...
				if (receivedParams[0].equals("listEventAvailability")) {
					response = hlp.get_events(receivedParams[2],
							receivedParams[1], ottawa_db, "OTW");
	...
	public String listEventAvailability(String eventType, String managerID) {
		...
		disp_events = hlp.get_events(managerID, eventType, ottawa_db, "OTW");
		...
		if (hlp.checkEventType(eventType, ottawa_db)) {
			...
			if (eventID.substring(0, 3).equals("TOR")
				...
				if (hlp.checkOutsideEvents(displayEvents, customerID, eventID)) {
					...
					Helpers.ServerLog("OTW", "Number of Booking of Outside Events Reached");
					checkEvent=false;
				}
			}
			String message = "";
	...
	public String getBookingSchedule(String customerID) {
		...
		if (user_db.containsKey(customerID)) {
			...
			if (customerID.substring(0, 3).equalsIgnoreCase("OTW")) {
